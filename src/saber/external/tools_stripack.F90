...
!----------------------------------------------------------------------
subroutine addnod(mpl,nst,k,x,y,z,list,lptr,lend,lnew)
...
! Initialization
kk = k
if (kk<4) call mpl%abort(subr,'K<4')
km1 = kk-1
ist = nst
if (ist<1) ist = km1
p(1) = x(kk)
...
if (i3/=0) then
  l = i1
  if (eq(p(1),x(l)).and.eq(p(2),y(l)).and.eq(p(3),z(l))) call mpl%abort(subr,'colinear or duplicate nodes')
  l = i2
  if (eq(p(1),x(l)).and.eq(p(2),y(l)).and.eq(p(3),z(l))) call mpl%abort(subr,'colinear or duplicate nodes')
  l = i3
  if (eq(p(1),x(l)).and.eq(p(2),y(l)).and.eq(p(3),z(l))) call mpl%abort(subr,'colinear or duplicate nodes')
...
if (inf(output,0.0_kind_real)) go to 7
n1 = n2
if (n1/=nl) go to 4
call det(x(n0),y(n0),z(n0),x(nf),y(nf),z(nf),xp,yp,zp,output)
if (inf(output,0.0_kind_real)) go to 6
...
if (inf(abs(x(n0)*xp+y(n0)*yp+z(n0)*zp),1.0-4.0*eps)) then
   ...
   do
      ...
      if (inf(output,0.0_kind_real)) exit
      lp = lptr(lp)
      n1 = abs(list(lp))
      if (n1==nl) then
         i1 = 0
...
if (supeq(output,0.0_kind_real)) then
   ! N2 is the rightmost visible node if P Forward N2->N1 or NEXT Forward N2->N1. Set Q to (N2 X N1) X N2.
    ...
    q(2) = y(n1)-s12*y(n2)
    q(3) = z(n1)-s12*z(n2)
    if (sup(xp*q(1)+yp*q(2)+zp*q(3),0.0_kind_real).or.sup(x(next)*q(1)+y(next)*q(2)+z(next)*q(3),0.0_kind_real)) go to 11
...
if (nl==0) then
   ...
   if (supeq(output,0.0_kind_real)) then
      ...
      q(2) = y(n2)-s12*y(n1)
      q(3) = z(n2)-s12*z(n1)
      if (sup(xp*q(1)+yp*q(2)+zp*q(3),0.0_kind_real).or.sup(x(next)*q(1)+y(next)*q(2)+z(next)*q(3),0.0_kind_real)) go to 13
...
!----------------------------------------------------------------------
subroutine trmesh(mpl,n,x,y,z,list,lptr,lend,lnew,near,next,dist)
...
! Initialization
nn = n
if (nn<3) call mpl%abort(subr,'N < 3')
list = 0
lptr = 0
lend = 0
...
if (.not.output_1) then
   ...
   call mpl%abort(subr,'The first three nodes are colinear')
end if
...
do k=4,nn
   ...
   do
      ...
      ! Test for the distance from I to K less than the distance from I to J. Indistinguishability threshold for cross-plateform reproducibility
