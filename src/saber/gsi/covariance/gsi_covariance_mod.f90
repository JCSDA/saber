...
if (nchecks > 0) then  ! only run checks if data was passed in from JEDI
  gsi_jedi_grid_error = .false.
  gsi_nx = self%grid%iec - self%grid%isc + 1
  jedi_nx = nint(checks(1))
  if (gsi_nx /= jedi_nx) then
  ...
  do ix = 1, gsi_nx
    ...
    if(jedi_lon < 0.) jedi_lon = jedi_lon + 360.
...
! Arguments
class(gsi_covariance), intent(in) :: self
...
else if (fields%has("eastward_wind").and.fields%has("northward_wind")) then
...
! --------------------------------------------------------------------------------------------------
   ...
   real(kind=kind_real), pointer, intent(inout) :: rank2(:,:)
   type(atlas_field) :: afield
...
!  endif
   ...
   if(mylon2*mylat2<=sizeofrank) then  !in global domain, or regional, the subdomains are of the laterary boundaries
