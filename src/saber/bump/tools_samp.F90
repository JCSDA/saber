...
logical :: lfull_grid,lverbosity,update,retry
logical,allocatable :: lmask(:)
character(len=6) :: gridid
character(len=1024),parameter :: subr = 'initialize_sampling_local'
...
end subroutine initialize_sampling_local
...
character(len=1024),parameter :: subr = 'initialize_sampling_global'
type(tree_type) :: tree
...
if (lfast) then
   ...
   do is1_glb_eff=2,ns1_glb_eff
      if (lmask(is1_glb_eff)) then
         cdf(is1_glb_eff) = cdf(is1_glb_eff-1)+1.0/rh1_glb_tmp(is1_glb_eff)**2
      end if
   end do
...
end subroutine initialize_sampling_global
