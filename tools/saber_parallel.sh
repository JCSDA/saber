...
echo "Tests start at ${initial_time}" > saber_ctest_log/summary.log

# List tests
list_get=`ctest -N | grep get_saber | gawk '{print $(NF)}'`
list=`ctest -N | grep test_bump | gawk '{print $(NF)}'`
...
for run in ${list}; do
   ...
   # Print info
   echo "   At this time, ${navail} CPUs are available and ${mpixomp} are required"
   ...
   while [ ${navail} -lt ${mpixomp} ]; do
      ...
      for crun in "${!pids[@]}"; do
         ...
         if ! kill -0 ${pids[${crun}]} 2>/dev/null; then
            ...
            echo "   Process ${crun} is done, ${navail} are now are available"

            # Check if this process succeed
            msg=`tail -n 1 saber_ctest_log/${crun}.log | gawk '{print $1}'`
            if test "${msg}" = "PASSED" ; then
               # Run succeed
               echo "${crun} succeed" >> saber_ctest_log/summary.log
               stest=$((stest+1))
            else
               # Run failed
               echo "${crun} failed" >> saber_ctest_log/summary.log
               ftest=$((ftest+1))
            fi
         fi
      done
   ...
   # Fire!
   echo "   Process ${run} is launched on CPUs ${cpu_list}"
   full_cmd="${cmd} ${cpu_list} > saber_ctest_log/${run}.log 2> saber_ctest_log/${run}.err &"
   eval ${full_cmd}
   pids[${run}]=$!
done
...
while [ ${#pids[@]} -gt 0 ]; do
   ...
   for crun in "${!pids[@]}"; do
      ...
      if ! kill -0 ${pids[${crun}]} 2>/dev/null; then
         ...
         # Check if this process succeed
         msg=`tail -n 1 saber_ctest_log/${crun}.log`
         if test "${msg}" = "PASSED" ]; then
            # Run succeed
            echo "${crun} succeed" >> saber_ctest_log/summary.log
            stest=$((stest+1))
...
echo "Tests finished at ${final_time}" >> saber_ctest_log/summary.log

# Elapsed time
elapsed_time=$((final_time_sec-initial_time_sec))
echo "Elapsed time: ${elapsed_time} sec." >> saber_ctest_log/summary.log
echo "${stest} run tests succeed and ${ftest} failed" >> saber_ctest_log/summary.log
tail -n 2 saber_ctest_log/summary.log
