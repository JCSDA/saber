...
#      xp=7H8H
      ...
      for timeslot in "02" "03" "04" ; do
         ln -sf ${modeldata}/${test}/${xp}/${date}/member_${i3}/forecast/ICMSHAROM+00${timeslot}.nc ${testdata}/${test}_ens1_${timeslot}_${i4}.nc
      done
   fi
...
#      date=20131220H12A
      for timeslot in "00" "06" ; do
         ln -sf ${modeldata}/${test}/${xp}/${date}/ensemble4D/${i3}/ICMSHARPE+00${timeslot}.nc ${testdata}/${test}_ens1_${timeslot}_${i4}.nc
      done
   fi
   ...
   if test ${test} = "bump_gem" ; then
      ne=10
      date=2014101706
      ln -sf ${modeldata}/${test}/${date}_006_${i4}.nc ${testdata}/${test}_ens1_00_${i4}.nc
      ...
      if test ${i} = 1 ; then
         ...
         for string in 'kfc' 'kuo' ; do
            for string2 in 'BLAC62' 'BOUJO' ; do
               ...
               typeset -RZ4 k4
               while [ ${k4} -le 64 ] ; do
                  ln -sf  ${modeldata}/${test}/member_${string}_${string2}_${k4}.nc ${testdata}/${test}_ens1_00_${j4}_${k4}.nc
                  let k4=k4+1
               done
               let j4=j4+1
            done
   ...
   if test ${test} = "bump_geos" ; then
      ...
      typeset -RZ2 ts
      while [ ${ts} -lt 13 ] ; do
         ln -sf ${modeldata}/${test}/mem${i3}/e200_C180.prog.eta.20180415_${ts}z.nc4 ${testdata}/${test}_ens1_${ts}_${i4}.nc
         let ts=ts+1
      done
   fi
   ...
   if test ${test} = "bump_gfs" ; then
      ne=10
      date=2014040100
      ln -sf ${modeldata}/${test}/sfg_${date}_fhr06s_mem${i3}.nc4 ${testdata}/${test}_ens1_00_${i4}.nc
   fi

   # IFS
   if test ${test} = "bump_ifs" ; then
      ne=10
      ln -sf ${modeldata}/${test}/member_${i}.nc ${testdata}/${test}_ens1_01_${i4}.nc
   fi

   # MPAS
   if test ${test} = "bump_mpas" ; then
      ne=10
      ln -sf ${modeldata}/${test}/x1.40962.output.2012-06-25_21.00.00.e${i}.nc ${testdata}/${test}_ens1_01_${i4}.nc
   fi

   # NEMO
   if test ${test} = "bump_nemovar" ; then
      ne=19
      ln -sf ${modeldata}/${test}/ENSEMBLES/ECMWF/goqu/opa${i}/goqu_20110605_000000_restart.nc ${testdata}/${test}_ens1_01_${i4}.nc
   fi
   if test ${test} = "bump_cera-20c" ; then
      ...
      for date in "20090215" "20090216" "20090217" "20090218" "20090219" "20090221" "20090222" "20090223" "20090224" ; do
         ln -sf ${modeldata}/${test}/CERA-20C/member_${date}+00_${i}.nc ${testdata}/${test}_ens1_01_${j4}.nc
         let j4=j4+9
      done
   fi
   ...
   if test ${test} = "bump_res" ; then
      ne=101
      ln -sf ${modeldata}/${test}/Ens_${i}.nc ${testdata}/${test}_ens1_01_${i4}.nc
   fi
   ...
   if test ${test} = "bump_wrf" ; then
      ...
      ln -sf ${modeldata}/${test}/wrfout_d01_${date}.${i3} ${testdata}/${test}_ens1_01_${i4}.nc
   fi
...
if test ${test} = "bump_geos" ; then
   ...
   typeset -RZ2 ts
   while [ ${ts} -lt 12 ] ; do
      ln -sf ${modeldata}/${test}/avg/e200_C180.prog.eta.20180415_${ts}z.nc4 ${testdata}/${test}_wind_${ts}.nc
      let ts=ts+1
   done
   ln -sf ${modeldata}/${test}/avg/e200_C180.prog.eta.20180416_00z.nc4 ${testdata}/${test}_wind_${ts}.nc
fi
...
if test ${test} = "bump_gem" ; then
   ...
   origin=${testdata}/${test}_ens1_00_0001.nc
   grid=${testdata}/${test}_grid.nc
   rm -f ${grid}
...
if test ${test} = "bump_geos" ; then
   ...
   origin=${testdata}/${test}_wind_00.nc
   grid=${testdata}/${test}_grid.nc
   rm -f ${grid}
...
if test ${test} = "bump_ifs" ; then
   ...
   origin=${testdata}/${test}_ens1_01_0001.nc
   grid=${testdata}/${test}_grid.nc
   rm -f ${grid}
