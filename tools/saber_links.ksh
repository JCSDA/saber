...
datadir=${HOME}/data
model=nemo
saberdir=${HOME}/code/saber/test/testdata
test=bump_nemovar
...
#      xp=7H8H
      ...
      for timeslot in "02" "03" "04" ; do
         ln -sf ${datadir}/${model}/${xp}/${date}/member_${i3}/forecast/ICMSHAROM+00${timeslot}.nc ${saberdir}/${test}_ens1_${timeslot}_${i4}.nc
      done
   fi
...
#      date=20131220H12A
      for timeslot in "00" "06" ; do
         ln -sf ${datadir}/${model}/${xp}/${date}/ensemble4D/${i3}/ICMSHARPE+00${timeslot}.nc ${saberdir}/${test}_ens1_${timeslot}_${i4}.nc
      done
   fi
   ...
   if test ${model} = "fv3" ; then
      ...
      ln -sf ${datadir}/${model}/ens1_00_${i4}.nc ${saberdir}/${test}_ens1_01_${i4}.nc
   fi
   ...
   if test ${model} = "gem" ; then
      ne=10
      date=2014101706
      ln -sf ${datadir}/${model}/${date}_006_${i4}.nc ${saberdir}/${test}_ens1_00_${i4}.nc
      ...
      if test ${i} = 1 ; then
         ...
         for string in 'kfc' 'kuo' ; do
            for string2 in 'BLAC62' 'BOUJO' ; do
               ...
               typeset -RZ4 k4
               while [ ${k4} -le 64 ] ; do
                  ln -sf  ${datadir}/${model}/member_${string}_${string2}_${k4}.nc ${saberdir}/${test}_ens1_00_${j4}_${k4}.nc
                  let k4=k4+1
               done
               let j4=j4+1
            done
   ...
   if test ${model} = "geos" ; then
      ...
      typeset -RZ2 ts
      while [ ${ts} -lt 24 ] ; do
         ln -sf ${datadir}/${model}/mem${i3}/e200_C180.prog.eta.20180415_${ts}z.nc4 ${saberdir}/${test}_ens1_${ts}_${i4}.nc
         let ts=ts+1
      done
      ln -sf ${datadir}/${model}/mem${i3}/e200_C180.prog.eta.20180416_00z.nc4 ${saberdir}/${test}_ens1_${ts}_${i4}.nc
   fi
   ...
   if test ${model} = "gfs" ; then
      ne=10
      date=2014040100
      ln -sf ${datadir}/${model}/sfg_${date}_fhr06s_mem${i3}.nc4 ${saberdir}/${test}_ens1_00_${i4}.nc
   fi

   # IFS
   if test ${model} = "ifs" ; then
      ne=10
      ln -sf ${datadir}/${model}/member_${i}.nc ${saberdir}/${test}_ens1_01_${i4}.nc
   fi

   # MPAS
   if test ${model} = "mpas" ; then
      ne=10
      ln -sf ${datadir}/${model}/x1.40962.output.2012-06-25_21.00.00.e${i}.nc ${saberdir}/${test}_ens1_01_${i4}.nc
   fi
...
#      xp=cera-20c
      if test ${xp} = "nemovar" ; then
         ne=19
         ln -sf ${datadir}/${model}/ENSEMBLES/ECMWF/goqu/opa${i}/goqu_20110605_000000_restart.nc ${saberdir}/${test}_ens1_01_${i4}.nc
      fi
      if test ${xp} = "cera-20c" ; then
         ...
         for date in "20090215" "20090216" "20090217" "20090218" "20090219" "20090221" "20090222" "20090223" "20090224" ; do
            ln -sf ${datadir}/${model}/CERA-20C/member_${date}+00_${i}.nc ${saberdir}/${test}_ens1_01_${j4}.nc
            let j4=j4+9
         done
      fi
   fi
   ...
   if test ${model} = "res" ; then
      ne=101
      ln -sf ${datadir}/${model}/Ens_${i}.nc ${saberdir}/${test}_ens1_01_${i4}.nc
   fi
   ...
   if test ${model} = "wrf" ; then
      ...
      ln -sf ${datadir}/${model}/wrfout_d01_${date}.${i3} ${saberdir}/${test}_ens1_01_${i4}.nc
   fi
...
if test ${model} = "geos" ; then
   ...
   typeset -RZ2 ts
   while [ ${ts} -lt 24 ] ; do
      ln -sf ${datadir}/${model}/avg/e200_C180.prog.eta.20180415_${ts}z.nc4 ${saberdir}/${test}_wind_${ts}.nc
      let ts=ts+1
   done
   ln -sf ${datadir}/${model}/avg/e200_C180.prog.eta.20180416_00z.nc4 ${saberdir}/${test}_wind_${ts}.nc
fi
...
if test ${model} = "gem" ; then
   ...
   origin=${saberdir}/${test}_ens1_00_0001.nc
   grid=${saberdir}/${test}_grid.nc
   rm -f ${grid}
...
if test ${model} = "geos" ; then
   ...
   origin=${saberdir}/${test}_wind_00.nc
   grid=${saberdir}/${test}_grid.nc
   rm -f ${grid}
...
if test ${model} = "ifs" ; then
   ...
   origin=${saberdir}/${test}_ens1_01_0001.nc
   grid=${saberdir}/${test}_grid.nc
   rm -f ${grid}
...
# MPAS
if test ${model} = "mpas" ; then
   origin=${datadir}/${model}/x1.40962.init.2018-04-15_00.00.00.nc
   grid=${saberdir}/${test}_grid.nc
   rm -f ${grid}
